...
import useTrialRequestHandlers from './modules/useTrialRequestHandlers';
import useFormValidation from './modules/useFormValidation';
import useTrialRequestSubmission from './modules/useTrialRequestSubmission';
import useTrialRequestData from './modules/useTrialRequestData';
import useCloseConfirmation from '../../../../hooks/useCloseConfirmation';
import useCopyPaste from '../../../../hooks/useCopyPaste';
import { useState, useEffect, useCallback } from 'react';

const useTrialRequestForm = (order, onClose, onOrderCreated, onOrderUpdated, viewMode = false, clientId) => {
  ...
  const { 
    ...
    addContact, 
    removeContact 
  } = useTrialRequestHandlers(formData, setFormData, errors, setErrors, viewMode);

  // Chargement des données de la demande d'essai en mode édition
  useTrialRequestData(
    order, 
    setFormData, 
    setMessage, 
  ...
  const formatForApi = useCallback((data) => {
    ...
    const formattedDate = data.request_date ? new Date(data.request_date).toISOString().split('T')[0] : null;
  ...
  const { handleSubmit } = useTrialRequestSubmission(
    formData, 
    parentId,
    setFormData, 
    validate,
...
export default useTrialRequestForm;
