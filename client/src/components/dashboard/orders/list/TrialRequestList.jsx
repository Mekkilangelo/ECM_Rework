...
const TrialRequestList = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: t('orders.date'),
      cellClassName: 'text-center',
      render: (order) => order.trialRequest?.request_date
        ? new Date(order.trialRequest?.request_date).toLocaleString('fr-FR', {
          day: '2-digit',
          month: '2-digit',
          year: 'numeric'
        })
        : t('common.unknown'),
      sortValue: (order) => order.trialRequest?.request_date ? new Date(order.trialRequest?.request_date).getTime() : 0
    },
    {
      key: 'modified_at',
  ...
  return (
    <>
      ...
      {/* Modal pour voir les détails - utilise TrialRequestForm en mode lecture seule */}
      <Modal
