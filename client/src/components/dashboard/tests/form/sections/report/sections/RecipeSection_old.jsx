...
const chartOptions = {
  ...
  const estimateSectionHeight = (sectionType, data) => {
    const baseHeight = 150; // En-tête et padding de base
    
    switch (sectionType) {
      case 'generalParams':
        return 200; // Section des paramètres généraux (taille fixe)
      
      case 'thermalCycle':
        if (!data || !data.thermal_cycle) return 0;
        const thermalRows = data.thermal_cycle.length;
        return 250 + (thermalRows * 35); // En-tête + tableau + lignes
      
      case 'chemicalCycle':
        if (!data || !data.chemical_cycle) return 0;
        const chemicalRows = data.chemical_cycle.length;
        return 250 + (chemicalRows * 35); // En-tête + tableau + lignes
      
      case 'chart':
        return 500; // Graphique avec padding
      
      case 'quench':
        if (!quenchData || (!quenchData.oil_quench && !quenchData.gas_quench)) return 0;
        let quenchHeight = 200; // En-tête de base
        
        if (quenchData.oil_quench && Object.keys(quenchData.oil_quench).length > 0) {
          quenchHeight += 300; // Section oil quench
        ...
        if (quenchData.gas_quench && Object.keys(quenchData.gas_quench).length > 0) {
          quenchHeight += 300; // Section gas quench
  ...
  const organizeContentInPages = useMemo(() => {
    ...
    const maxPageHeight = 800; // Hauteur disponible sur une page A4 (297mm - marges)
    const headerHeight = 120; // Hauteur du header de section
    ...
    sections.forEach(section => {
      ...
      if (sectionHeight === 0) return; // Skip empty sections
      
      // Si la section ne rentre pas dans la page actuelle, créer une nouvelle page
      ...
      currentPage.estimatedHeight += sectionHeight + 30; // 30px d'espacement entre sections
    });
