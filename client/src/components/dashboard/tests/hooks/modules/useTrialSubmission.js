...
/**
 * Hook spécifique pour gérer les soumissions de trials
 * @param {Object} formData - Données du formulaire de trial
 ...
 * @param {Object} trial - Trial existant (pour le mode édition)
 ...
 * @param {Function} onTrialCreated - Callback après création * @param {Function} onTrialUpdated - Callback après mise à jour
...
const useTrialSubmission = (
          ...
          result.samples.map((sample, sampleIdx) => {
            ...
            if (process.env.NODE_ENV === 'development') {
              ...
              if (curveData?.distances && curveData?.series) {
                ...
                console.log('⚠️ Format LEGACY points détecté:', curveData.points[0]);
              }
            }
            return {
              step: sample.step,
  ...
  return useApiSubmission({
    ...
    setLoading,
    setMessage,
    onCreated: onTrialCreated,
    onUpdated: onTrialUpdated,
    onClose,
    formatDataForApi,
    customApiService: {
...
export default useTrialSubmission;